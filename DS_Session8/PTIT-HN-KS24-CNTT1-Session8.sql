CREATE DATABASE mini_project_ss08;
USE mini_project_ss08;

-- Xóa bảng nếu đã tồn tại (để chạy lại nhiều lần)
DROP TABLE IF EXISTS bookings;
DROP TABLE IF EXISTS rooms;
DROP TABLE IF EXISTS guests;

-- Bảng khách hàng
CREATE TABLE guests (
    guest_id INT PRIMARY KEY AUTO_INCREMENT,
    guest_name VARCHAR(100),
    phone VARCHAR(20)
);

-- Bảng phòng
CREATE TABLE rooms (
    room_id INT PRIMARY KEY AUTO_INCREMENT,
    room_type VARCHAR(50),
    price_per_day DECIMAL(10,0)
);

-- Bảng đặt phòng
CREATE TABLE bookings (
    booking_id INT PRIMARY KEY AUTO_INCREMENT,
    guest_id INT,
    room_id INT,
    check_in DATE,
    check_out DATE,
    FOREIGN KEY (guest_id) REFERENCES guests(guest_id),
    FOREIGN KEY (room_id) REFERENCES rooms(room_id)
);

INSERT INTO guests (guest_name, phone) VALUES
('Nguyễn Văn An', '0901111111'),
('Trần Thị Bình', '0902222222'),
('Lê Văn Cường', '0903333333'),
('Phạm Thị Dung', '0904444444'),
('Hoàng Văn Em', '0905555555');

INSERT INTO rooms (room_type, price_per_day) VALUES
('Standard', 500000),
('Standard', 500000),
('Deluxe', 800000),
('Deluxe', 800000),
('VIP', 1500000),
('VIP', 2000000);

INSERT INTO bookings (guest_id, room_id, check_in, check_out) VALUES
(1, 1, '2024-01-10', '2024-01-12'), -- 2 ngày
(1, 3, '2024-03-05', '2024-03-10'), -- 5 ngày
(2, 2, '2024-02-01', '2024-02-03'), -- 2 ngày
(2, 5, '2024-04-15', '2024-04-18'), -- 3 ngày
(3, 4, '2023-12-20', '2023-12-25'), -- 5 ngày
(3, 6, '2024-05-01', '2024-05-06'), -- 5 ngày
(4, 1, '2024-06-10', '2024-06-11'); -- 1 ngày

-- Liệt kê tên khách và số điện thoại của tất cả khách hàng
select * from guests;
-- Liệt kê các loại phòng khác nhau trong khách sạn
select room_type from rooms group by room_type;
-- Hiển thị loại phòng và giá thuê theo ngày, sắp xếp theo giá tăng dần
select room_type, price_per_day from rooms 
group by room_type, price_per_day 
order by price_per_day asc;
-- Hiển thị các phòng có giá thuê lớn hơn 1.000.000
select room_type from rooms
where price_per_day > 1000000
group by room_type;
-- Liệt kê các lần đặt phòng diễn ra trong năm 2024
select * from bookings where check_in like '%2024%';
-- Cho biết số lượng phòng của từng loại phòng
select room_type, count(*)
from rooms
group by room_type;
-- Hiện thị danh sách đặt phòng
select g3.guest_name, g2.room_type, g1.check_in
from bookings g1
left join rooms g2 on g2.room_id = g1.room_id
left join guests g3 on g3.guest_id = g1.guest_id;
-- Hiện thị số phòng khách đặt
select g2.guest_name, count(*)
from bookings g1
left join guests g2 on g2.guest_id = g1.guest_id
group by g1.guest_id;
-- Hiện thị doanh thu mỗi phòng
select g1.room_id ,sum((g1.check_out - g1.check_in) * g2.price_per_day) as 'Doanh thu'
from bookings g1
left join rooms g2 on g2.room_id = g1.room_id
group by g1.room_id
order by g1.room_id asc;
-- Hiện thị tổng doanh thu
select g2.room_type ,sum((g1.check_out - g1.check_in) * g2.price_per_day) as 'Doanh thu'
from bookings g1
left join rooms g2 on g2.room_id = g1.room_id
group by g2.room_type;
-- Hiện thị khách đặt từ 2 lần
select g2.guest_name, count(*)
from bookings g1
left join guests g2 on g2.guest_id = g1.guest_id
group by g1.guest_id
having count(*) >=2;
-- Hiện thị phòng có số lần đặt nhiều nhất
select g2.room_type, count(*)
from bookings g1
left join rooms g2 on g2.room_id = g1.room_id
group by g2.room_type
order by count(*) desc
limit 1;